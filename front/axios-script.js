...
async function getDataSet(category) {
  ...
  const dataSet = await axios({
    ...
    url: `http://100.21.154.29:3000/restaurants/?category=${qs}`,
...
function getCoordsByAddress(address) {
  return new Promise((resolve, reject) => {
    ...
    geocoder.addressSearch(address, function (result, status) {
      ...
      reject(new Error('getCoordsByAddress not valid address'));
    });
  });
}
...
function getContent(data) {
  ...
  findUrl = replaceUrl.split('&')[0];

  // 인포윈도우 가공하기
  return `<div class="infowindow">
  <div class="infowindow-img-container">
    <img src=https://img.youtube.com/vi/${findUrl}/mqdefault.jpg class="infowindow-img">
  </div>
